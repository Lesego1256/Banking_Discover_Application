...
LEFT JOIN (Select cpso.client_id As CLIENT_ID ,sum( ca.DISPLAY_BALANCE) AS LOAN_AMOUNT



FROM CLIENT cpso

join CLIENT_ACCOUNT ca on  cpso.CLIENT_ID = ca.CLIENT_ID

join ACCOUNT_TYPE AT on ca.ACCOUNT_TYPE_CODE = AT.ACCOUNT_TYPE_CODE AND AT.ACCOUNT_TYPE_CODE  IN ('PLOAN', 'HLOAN')

Group by cpso.CLIENT_id) as Table1
